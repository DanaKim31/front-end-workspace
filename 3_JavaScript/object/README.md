...
```js
const regExp1 = new RegExp("패턴"); // RegExp! 함수는 RegExp로 정해져 있음
const regExp2 = /패턴/; // 리터럴!
```

### RegExp 객체에서 제공하는 함수

- 정규식.test(문자열) : 문자열에서 정규식 값과 일치하는 값이 있으면 true, 일치하는 값이 없으면 false를 리턴
- 정규식.exec(문자열) : 문자열에서 정규식 값과 일치하는 값이 있으면 처음 매칭된 문자열을 리턴, 일치하는 값이 없으면 null 리턴

### 문자열(String 객체) 객체에서 제공하는 함수

- 문자열.match(정규식) : 문자열에서 정규식 값과 일치하는 값을 찾아서 리턴
- 문자열.search(정규식) : 문자열에서 정규식 값과 일치하는 값의 시작 index 값을 리턴, 일치하는 값이 없다면 -1 리턴
...
1. i : 대소문자를 가리지 않고 비교한다. (많이 쓰임!)
2. g : 전역으로(전체 문자열에서) 비교를 수행한다. (많이 쓰임!)
3. m : 여러 줄의 검사를 수행한다.
...
- [] : [] 내에 문자들 중 하나라도 존재할 경우를 의미 (많이 쓰임!)

### 임의의 문자열 검색

- . : 임의의 문자 한 개(문자의 개수)를 의미하고, 문자 내용은 무엇이든 상관없다.

### 추가 메타 문자 (패턴 내에서 의미들이 부여되어 있는 문자)

- \d : 숫자를 의미
- \D : 숫자가 아닌 문자를 의미(띄어쓰기 포함)
